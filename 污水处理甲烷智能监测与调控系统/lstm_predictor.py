...
class CarbonLSTMPredictor:
    ...
    def _convert_to_monthly(self, daily_df):
        ...
        if not monthly_df.empty and 'total_CO2eq' in monthly_df.columns:
            ...
            print(f"LSTM转换: 标准化月度甲烷排放 = {avg_monthly_emission:.1f} kgCO2eq/月")
    ...
    def predict(self, df, target_column='total_CO2eq', steps=12):
        ...
        if not TENSORFLOW_AVAILABLE or self.model is None:
            logger.info("使用备用统计预测模式")
            return self._fallback_predict(df, target_column, steps)
        ...
        if not scaler_fitted:
            ...
            if len(target_values) > 0:
                ...
                # 如果没有目标数据，使用默认范围
                self.target_scaler.fit([[0], [2000]])  # 假设甲烷排放在0-2000范围内
    ...
    def _fallback_predict(self, df, target_column='total_CO2eq', steps=12):
        """备用预测方法 - 基于历史统计（当TensorFlow不可用时使用）"""
        # 转换为月度数据（如果需要）
        if '年月' not in df.columns:
            df = self._convert_to_monthly(df)
        ...
        for i in range(1, steps + 1):
            ...
            lower_bounds.append(max(historical_mean * 0.05, pred - error_estimate))
            upper_bounds.append(pred + error_estimate)
        ...
        logger.info(f"备用预测完成：生成 {steps} 个月的预测")
        return result_df
